...
#import "CustomSecondBarButton.h"

@interface CustomSecondTabBar ()<UIAlertViewDelegate>

// 发布需求按钮
@property (nonatomic, weak) UIButton *plusBtn;
@property (nonatomic, weak) UILabel *plusLabel;
@property (nonatomic, strong) NSMutableArray *tabBarButtons;
@property (nonatomic, weak) CustomSecondBarButton *currentSelectedBtn;
...
@implementation CustomSecondTabBar
...
{
    // 1.创建按钮
    CustomSecondBarButton *tabBarButton = [[CustomSecondBarButton alloc] init];
...
#pragma mark -- 按钮事件
-(void)btnOnClick:(CustomSecondBarButton *)btn;
{
    self.btnTag = btn.tag;
    if (self.btnTag == 0) {
        NSLog(@"点击第一个控制器");
...
-(void)selectTabItem:(NSInteger)index{
    CustomSecondBarButton *btn= _tabBarButtons[index];
    [self btnOnClick:btn];
}
